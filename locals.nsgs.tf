locals {
  ...
  destination_subnets_to_address_spaces = {
    for rule_name, rule in local.base_security_rules :
    rule_name => tostring(local.network_address_spaces[rule.to.subnet.network_key_reference].subnets[rule.to.subnet.subnet_key_reference].address_space)
  ...
  source_subnets_to_address_spaces = {
    for rule_name, rule in local.base_security_rules :
    rule_name => tostring(local.network_address_spaces[rule.from.subnet.network_key_reference].subnets[rule.from.subnet.subnet_key_reference].address_space)
  ...
  network_security_groups = {
    for nsg_key, nsg in var.network_security_groups : nsg_key => {
      ...
      resource_group_key_reference = nsg.resource_group_key_reference
      tags                         = nsg.tags

      lock = (
        length([
          for group in nsg.lock_groups_key_reference :
