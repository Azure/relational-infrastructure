locals {
  ...
  #     disk_name => (
  #       (length(disk.lock_groups_key_reference) > 0) &&
  #       (anytrue([
  ...
  key_vault_locks = {
    ...
    vault_name => (
      ...
      ? (
        anytrue(([
          for group_name in key_vault.lock_groups_key_reference :
  ...
  network_locks = {
    ...
    network_key => (
      ...
      ? (
        anytrue(([
          for group_name in network.lock_groups_key_reference :
          var.lock_groups[group_name].read_only
        ]))
        ? local.avm_convention_locks.read_only
        : local.avm_convention_locks.no_delete
  ...
  resource_group_locks = {
    ...
    resource_group_key => (
      ...
      ? (
        anytrue(([
          for lock_group_name in resource_group.lock_groups_key_reference :
  ...
  storage_account_locks = {
    ...
    account_name => (
      ...
      ? (
        anytrue(([
          for group_name in account.lock_groups_key_reference :
          var.lock_groups[group_name].read_only
        ]))
        ? local.avm_convention_locks.read_only
        : local.avm_convention_locks.no_delete
  ...
  #       ? (
  #         anytrue(([
  #           for group_name in disk.lock_groups_key_reference :
  #           var.lock_groups[group_name].read_only
  #         ]))
  #         ? "read_only"
  #         : "no_delete"
  ...
  network_interface_locks = {
    ...
    vm_set_name => {
      ...
      nic_name => (
        ...
        ? (
          anytrue(([
            for group_name in nic.lock_groups_key_reference :
            var.lock_groups[group_name].read_only
          ]))
          ? "read_only"
          : "no_delete"
  ...
  virtual_machine_set_locks = {
    ...
    vm_set_name => (
      ...
      ? (
        anytrue(([
          for group_name in vm_set.lock_groups_key_reference :
          var.lock_groups[group_name].read_only
        ]))
        ? "read_only"
        : "no_delete"
