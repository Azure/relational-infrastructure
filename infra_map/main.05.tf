...
module "az_subscription_5_infra_map" {
  ...
  count  = local.deploy_to_subscription[local._s5]
  ...
  default_resource_group_key      = local.subscriptions_by_slot[local._s5].default_resource_group_key
  private_link_resource_group_key = local.subscriptions_by_slot[local._s5].private_link_resource_group_key
  ...
  networks = {
    ...
    if local.subscription_slots[network.subscription_key] == local._s5
  }

  private_dns_zones = {
    for dns_zone_name, dns_zone in var.private_dns_zones : dns_zone_name => dns_zone
    if local.subscription_slots[dns_zone.subscription_key] == local._s5
  }

  resource_groups = {
    for group_name, group in var.resource_groups
    : group_name => group
    if local.subscription_slots[group.subscription_key] == local._s5
  }

  storage_accounts = {
    for account_name, account in var.storage_accounts
    : account_name => account
    if local.subscription_slots[account.subscription_key] == local._s5
  }

  blob_containers = {
    for container_key, container in var.blob_containers
    : container_key => container
    if local.subscription_slots[var.storage_accounts[container.storage_account_key].subscription_key] == local._s5
  }

  file_shares = {
    for share_key, share in var.file_shares
    : share_key => share
    if local.subscription_slots[var.storage_accounts[share.storage_account_key].subscription_key] == local._s5
  }

  virtual_machine_set_specs = {
    for vm_set_name, vm_set_specs in var.virtual_machine_set_specs
    : vm_set_name => vm_set_specs
    if try(local.subscription_slots[var.virtual_machine_sets[vm_set_name].subscription_key], null) == local._s5
  }
  ...
  key_vaults = {
    ...
    if local.subscription_slots[key_vault.subscription_key] == local._s5
  }

  private_endpoints = {
    key_vaults = {
      for pe_name, pe in var.private_endpoints.key_vaults : pe_name => pe
      if local.subscription_slots[var.key_vaults[pe.key_vault_key].subscription_key] == local._s5
    }

    blob_containers = {
      for pe_name, pe in var.private_endpoints.blob_containers : pe_name => pe
      if local.subscription_slots[var.storage_accounts[var.blob_containers[pe.container_key].storage_account_key].subscription_key] == local._s5
    }

    file_shares = {
      for pe_name, pe in var.private_endpoints.file_shares : pe_name => pe
      if local.subscription_slots[var.storage_accounts[var.file_shares[pe.share_key].storage_account_key].subscription_key] == local._s5
    }
  }
}
...
resource "azurerm_virtual_network_peering" "az_subscription_5_peerings" {
  for_each = {
    for pair in flatten([
      for from_name, from_network in var.networks : [
        ...
        if from_network.subscription_key == try(local.subscription_names_by_slot[local._s5], null)
      ]
    ]) : pair.key => pair
  }
  ...
  name                 = each.key
  resource_group_key  = try(module.az_subscription_5_infra_map[0].resource_groups[var.networks[each.value.from_name].resource_group_key].resource_name, null)
  virtual_network_name = try(module.az_subscription_5_infra_map[0].networks[each.value.from_name].resource_name, null)
