...
module "az_subscription_2_infra_map" {
  ...
  storage_accounts = {
    ...
    if var.subscriptions[account.subscription_name].subscription_slot == local._s2
  }

  blob_containers = {
    for container_name, container in var.blob_containers
    : container_name => container
    if var.subscriptions[var.storage_accounts[container.storage_account_name].subscription_name].subscription_slot == local._s2
  }

  file_shares = {
    for share_name, share in var.file_shares
    : share_name => share
    if var.subscriptions[var.storage_accounts[share.storage_account_name].subscription_name].subscription_slot == local._s2
  }
