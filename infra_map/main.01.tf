...
module "az_subscription_1_infra_map" {
  ...
  virtual_machine_set_specs = {
    ...
    if local.subscription_slots[var.virtual_machine_sets[vm_set_name].subscription_name] == local._s1
  }
  ...
  private_endpoints = {
    key_vaults = {
      ...
      : key_vault_name => key_vault
      if local.subscription_slots[var.key_vaults[key_vault_name].subscription_name] == local._s1
    }
