...
module "az_subscription_8_infra_map" {
  ...
  network_security_groups = {
    ...
    if local.subscription_slots[nsg.subscription_name] == local._s8
  }

  private_dns_zones = {
    for dns_zone_name, dns_zone in var.private_dns_zones : dns_zone_name => dns_zone
    if local.subscription_slots[dns_zone.subscription_name] == local._s8
  }
  ...
  virtual_machine_scale_sets = {
    ...
    if local.subscription_slots[scale_set.subscription_name] == local._s8
  }

  virtual_machine_set_specs = {
    for vm_set_name, vm_set_specs in var.virtual_machine_set_specs
    : vm_set_name => vm_set_specs
    if try(local.subscription_slots[var.virtual_machine_sets[vm_set_name].subscription_name], null) == local._s8
  }
  ...
  private_endpoints = {
    key_vaults = {
      ...
      if local.subscription_slots[var.key_vaults[pe.key_vault_name].subscription_name] == local._s8
    }

    blob_containers = {
      for pe_name, pe in var.private_endpoints.blob_containers : pe_name => pe
      if local.subscription_slots[var.storage_accounts[var.blob_containers[pe.container_name].storage_account_name].subscription_name] == local._s8
    }

    file_shares = {
      for pe_name, pe in var.private_endpoints.file_shares : pe_name => pe
      if local.subscription_slots[var.storage_accounts[var.file_shares[pe.share_name].storage_account_name].subscription_name] == local._s8
    }
  }
}
...
resource "azurerm_virtual_network_peering" "az_subscription_8_peerings" {
  for_each = {
    for pair in flatten([
      for from_name, from_network in var.networks : [
        ...
        if from_network.subscription_name == try(local.subscription_names_by_slot[local._s8], null)
      ]
    ]) : pair.key => pair
  }
  ...
  name                 = each.key
  resource_group_name  = try(module.az_subscription_8_infra_map[0].resource_groups[var.networks[each.value.from_name].resource_group_name].resource_name, null)
