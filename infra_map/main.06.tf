...
module "az_subscription_6_infra_map" {
  ...
  network_security_groups = {
    ...
    if local.subscription_slots[nsg.subscription_name] == local._s6
  }

  private_dns_zones = {
    for dns_zone_name, dns_zone in var.private_dns_zones : dns_zone_name => dns_zone
    if local.subscription_slots[dns_zone.subscription_name] == local._s6
  }
  ...
  virtual_machine_set_specs = {
    ...
    if try(local.subscription_slots[var.virtual_machine_sets[vm_set_name].subscription_name], null) == local._s6
  }
  ...
  private_endpoints = {
    key_vaults = {
      ...
      if local.subscription_slots[var.key_vaults[pe.key_vault_name].subscription_name] == local._s6
    }

    blob_containers = {
      for pe_name, pe in var.private_endpoints.blob_containers : pe_name => pe
      if local.subscription_slots[var.storage_accounts[var.blob_containers[pe.container_name].storage_account_name].subscription_name] == local._s6
    }

    file_shares = {
      for pe_name, pe in var.private_endpoints.file_shares : pe_name => pe
      if local.subscription_slots[var.storage_accounts[var.file_shares[pe.share_name].storage_account_name].subscription_name] == local._s6
    }
  }
}
