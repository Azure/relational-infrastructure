...
module "az_subscription_7_infra_map" {
  ...
  count  = try(local.subscriptions_by_slot[local._s7] != null ? 1 : 0, 0)
  ...
  private_endpoints = {
    ...
    blob_containers = {
      ...
      : container_name => container
      if vlocal.subscription_slots[var.storage_accounts[var.blob_containers[container_name].storage_account_name].subscription_name] == local._s7
    }
