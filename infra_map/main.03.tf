locals {
  _s3 = "az_subscription_3"
}

module "az_subscription_3_infra_map" {
  source = "../subscription_infra_map"
  count  = local.deploy_to_subscription[local._s3]

  providers = {
    azurerm = azurerm.az_subscription_3
  }
  ...
  tags = merge(
    ...
    var.include_label_tags ? {
      "subscription_label" = local.subscription_names_by_slot[local._s3]
    } : {}
  )

  default_resource_group_name      = local.subscriptions_by_slot[local._s3].default_resource_group_name
  private_link_resource_group_name = local.subscriptions_by_slot[local._s3].private_link_resource_group_name
  ...
  networks = {
    ...
    if local.subscription_slots[network.subscription_name] == local._s3
  }

  network_security_groups = {
    for nsg_name, nsg in var.network_security_groups : nsg_name => nsg
    if local.subscription_slots[nsg.subscription_name] == local._s3
  }

  private_dns_zones = {
    for dns_zone_name, dns_zone in var.private_dns_zones : dns_zone_name => dns_zone
    if local.subscription_slots[dns_zone.subscription_name] == local._s3
  }

  resource_groups = {
    for group_name, group in var.resource_groups
    : group_name => group
    if local.subscription_slots[group.subscription_name] == local._s3
  }

  storage_accounts = {
    for account_name, account in var.storage_accounts
    : account_name => account
    if local.subscription_slots[account.subscription_name] == local._s3
  }

  blob_containers = {
    for container_name, container in var.blob_containers
    : container_name => container
    if local.subscription_slots[var.storage_accounts[container.storage_account_name].subscription_name] == local._s3
  }

  file_shares = {
    for share_name, share in var.file_shares
    : share_name => share
    if local.subscription_slots[var.storage_accounts[share.storage_account_name].subscription_name] == local._s3
  }

  virtual_machine_sets = {
    for vm_set_name, vm_set in var.virtual_machine_sets
    : vm_set_name => vm_set
    if local.subscription_slots[vm_set.subscription_name] == local._s3
  }

  virtual_machine_scale_sets = {
    for scale_set_name, scale_set in var.virtual_machine_scale_sets
    : scale_set_name => scale_set
    if local.subscription_slots[scale_set.subscription_name] == local._s3
  }

  virtual_machine_set_specs = {
    for vm_set_name, vm_set_specs in var.virtual_machine_set_specs
    : vm_set_name => vm_set_specs
    if try(local.subscription_slots[var.virtual_machine_sets[vm_set_name].subscription_name], null) == local._s3
  }
  ...
  key_vaults = {
    ...
    if local.subscription_slots[key_vault.subscription_name] == local._s3
  }

  private_endpoints = {
    key_vaults = {
      for pe_name, pe in var.private_endpoints.key_vaults : pe_name => pe
      if local.subscription_slots[var.key_vaults[pe.key_vault_name].subscription_name] == local._s3
    }

    blob_containers = {
      for pe_name, pe in var.private_endpoints.blob_containers : pe_name => pe
      if local.subscription_slots[var.storage_accounts[var.blob_containers[pe.container_name].storage_account_name].subscription_name] == local._s3
    }

    file_shares = {
      for pe_name, pe in var.private_endpoints.file_shares : pe_name => pe
      if local.subscription_slots[var.storage_accounts[var.file_shares[pe.share_name].storage_account_name].subscription_name] == local._s3
    }
  }
}
...
resource "azurerm_virtual_network_peering" "az_subscription_3_peerings" {
  for_each = {
    for pair in flatten([
      for from_name, from_network in var.networks : [
        ...
        if from_network.subscription_name == try(local.subscription_names_by_slot[local._s3], null)
      ]
    ]) : pair.key => pair
  }
  ...
  name                 = each.key
  resource_group_name  = try(module.az_subscription_3_infra_map[0].resource_groups[var.networks[each.value.from_name].resource_group_name].resource_name, null)
  virtual_network_name = try(module.az_subscription_3_infra_map[0].networks[each.value.from_name].resource_name, null)
