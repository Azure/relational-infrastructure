...
module "az_subscription_3_infra_map" {
  ...
  virtual_machine_set_specs = {
    ...
    if local.subscription_slots[var.virtual_machine_sets[vm_set_name].subscription_name] == local._s3
  }
  ...
  private_endpoints = {
    key_vaults = {
      ...
      : key_vault_name => key_vault
      if local.subscription_slots[var.key_vaults[key_vault_name].subscription_name] == local._s3
    }

    blob_containers = {
      for container_name, container in var.private_endpoints.blob_containers
      : container_name => container
      if local.subscription_slots[var.storage_accounts[var.blob_containers[container_name].storage_account_name].subscription_name] == local._s3
    }

    file_shares = {
      for share_name, share in var.private_endpoints.file_shares
      : share_name => share
      if local.subscription_slots[var.storage_accounts[var.file_shares[share_name].storage_account_name].subscription_name] == local._s3
    }
  }
}
